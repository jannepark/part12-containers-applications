...
router.post('/', async (req, res) => {
  ...
  // Initialize added_todos to 0 if it does not exist or is not a valid number
  addedTodos = Number(addedTodos);
  if (isNaN(addedTodos)) {
    console.error('Expected a numeric value for added_todos, but got:', addedTodos);
    addedTodos = 0;
  }
...
// GET statistics at /statistics, which returns the number of added todos
router.get('/statistics', async (req, res) => {
  let added_todos = await getAsync('added_todos');
  added_todos = parseInt(added_todos, 10); // Convert to integer
  if (isNaN(added_todos)) {
    added_todos = 0; // Default to 0 if not a number
  }
